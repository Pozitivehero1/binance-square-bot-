# Binance Square Bot — Attention Edition
...
Cron запускается каждые 20 минут и задаёт реальную частоту публикаций. `ENABLE_PACING_LIMITS=0` отключает старый лимит 120 минут и дневной бюджет 6 постов. `ENABLE_REACH_GATE=0` не позволяет внутреннему баллу пропускать очередной cron-запуск.
...
- сохраняются 15 редакционных форматов и 8 визуальных семейств;
- генератор сначала пытается заполнить пакет разными форматами и только потом допускает повтор семейства;
- по умолчанию используется `CONTENT_MODE=deterministic`, чтобы снизить долю AI-похожей подачи;
- Mistral остаётся необязательным и не управляет ценами, направлением, уровнями или индикаторами.
...
При `ENABLE_PACING_LIMITS=0` значения глобального интервала и дневного лимита не ограничивают cron. `FORCE_PUBLISH=1` оставлен только для разовой диагностики.
...
`adaptive` публикует один визуал, соответствующий идее поста. Доступны `card`, `chart`, `both`, `none`.
