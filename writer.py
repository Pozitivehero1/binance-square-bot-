...
CTA_LIST = [
    "Какой уровень, по-вашему, будет достигнут первым: TP1 или стоп?",
    "Вы видите подтверждение сценария на своём графике?",
    "Какой уровень здесь считаете главным для отмены идеи?",
    "На каком таймфрейме этот сетап выглядит убедительнее?",
    "Что для вас важнее в этом сетапе: объём, тренд или структура?",
    "Сценарий продолжения или возврат в диапазон — какой вариант ближе?",
]
...
def generate_post_with_memory(
    *,
    ...
    sections = {
        ...
        "risk_note": (
            f"Оценка качества сетапа: {setup_score:.0f}/100. Это техническая оценка совпадения "
            "факторов, а не вероятность прибыли. Размер позиции рассчитывайте от допустимого риска.\n"
