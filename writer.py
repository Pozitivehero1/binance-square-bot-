...
POST_MAX_CHARS = int(os.getenv("POST_MAX_CHARS", "920"))
POST_MIN_CHARS = int(os.getenv("POST_MIN_CHARS", "260"))
...
def _fact_catalog(ind, mtf, direction: str, btc, angle: SignalAngle, levels: Dict[str, float]) -> Dict[str, str]:
...
def _tags(angle: SignalAngle, direction: str, variant_index: int) -> str:
    ...
    return angle_tag if variant_index % 3 == 2 else f"{angle_tag} {direction_tag}"
...
def _default_fact_ids(format_id: str, facts: Dict[str, str], variant_index: int = 0) -> List[str]:
    mapping = {
        "why_wait": ["setup", "volume", "vwap", "risk_math", "changes", "trend"],
        "level_story": ["range", "trend", "volume", "vwap", "mtf", "changes"],
        "two_scenarios": ["setup", "mtf", "risk_math", "range", "volume", "changes"],
        "risk_memo": ["risk_math", "range", "momentum", "volume", "mtf", "vwap"],
        "trader_journal": ["setup", "changes", "mtf", "range", "momentum", "volume"],
        "indicator_lesson": ["momentum", "trend", "vwap", "volume", "mtf", "changes"],
        "market_context": ["btc", "mtf", "changes", "trend", "volume", "range"],
        "data_brief": ["momentum", "volume", "changes", "vwap", "range", "risk_math"],
        "setup_plan": ["setup", "trend", "mtf", "volume", "range", "momentum"],
        "contrarian_take": ["volume", "vwap", "risk_math", "range", "mtf", "changes"],
        "mistake_to_avoid": ["risk_math", "vwap", "volume", "range", "changes", "mtf"],
        "execution_protocol": ["setup", "trend", "risk_math", "volume", "range", "momentum"],
        "signal_vs_trade": ["setup", "momentum", "vwap", "risk_math", "mtf", "volume"],
        "liquidity_map": ["range", "vwap", "volume", "changes", "risk_math", "trend"],
        "follow_up": ["changes", "range", "mtf", "risk_math", "trend", "volume"],
    }
...
def _render_post(
    *,
    ...
    plan = _plan_block(levels, compact=format_id in {"data_brief", "risk_memo"}, variant_index=variant_index)
    ...
    if format_id == "why_wait":
        ...
        blocks = (
            headline,
            "Риск считаю до входа, а не после первой неприятной свечи.",
            fact_text,
            plan,
...
def _headline_for(
    *, basic: str, direction: str, format_id: str, angle: SignalAngle, angle_copy: Dict[str, str], ind, levels: Dict[str, float], recent_titles: Sequence[str], index: int,
) -> str:
    risk_pct, reward_pct = _risk_reward_percentages(levels)
...
def _build_generated(
    *, basic: str, mtf, score, levels: Dict[str, float], btc, memory: Optional[PostMemory], format_id: str, angle: SignalAngle, index: int, ai: Optional[dict] = None,
    ...
    facts = _fact_catalog(ind, mtf, score.direction, btc, angle, levels)
    headline = _headline_for(
        basic=basic, direction=score.direction, format_id=format_id, angle=angle,
        angle_copy=angle_copy, ind=ind, levels=levels, recent_titles=recent_titles, index=index,
    )
...
def generate_post_candidates(
    *,
    ...
    if _content_mode() in {"ai", "ai_first", "mistral"} and _api_key():
        ...
        facts = _fact_catalog(ind, mtf, score.direction, btc, primary_angle, levels)
        try:
...
def generate_post_draft(
    *, symbol: str, basic: str, mtf, score, memory: Optional[PostMemory] = None,
    levels: Optional[Dict[str, float]] = None, btc=None, variant_index: int = 0,
) -> GeneratedPost:
    drafts = generate_post_candidates(
        symbol=symbol, basic=basic, mtf=mtf, score=score, memory=memory,
        levels=levels, btc=btc, variant_count=max(4, variant_index + 1),
    )
    if not drafts:
