...
def _test_mistral_full_author_and_fact_lock() -> None:
    ...
    payload = {
        "candidates": [
            ...
            {"format_id": "trade_map", "text": f"В $TEST цена у {e} — проверяю реакцию на рабочую зону\n\nДля продолжения движения нужно удержание уровня. Сам факт касания не означает, что покупатели уже закрепились; без этой реакции сценарий остаётся только условием."},
            ...
            {"format_id": "risk_first", "text": f"$TEST возле {e}: одного касания для сделки недостаточно\n\nСмотрю, сможет ли цена остаться в рабочей области. Если её покинет, основание для этого сценария исчезнет; заранее считать движение состоявшимся нельзя."},
