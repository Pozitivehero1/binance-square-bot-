...
class PostQualityEvaluator:
    MIN_LENGTH = int(os.getenv("POST_MIN_CHARS", "260"))
    MAX_LENGTH = int(os.getenv("POST_MAX_CHARS", "920"))
    ...
    def report(
        ...
        *,
        ...
        weights = {
            "factual_contract": 0.25,
            "headline": 0.18,
            "readability": 0.12,
            "editorial_structure": 0.14,
            "human_voice": 0.10,
            "engagement": 0.08,
            "credibility": 0.08,
            "spam_control": 0.05,
    ...
    def validate(
        ...
        *,
        ...
        if levels:
            ...
            if f"{levels.get('risk_reward', 0):.2f}" not in text:
    ...
    def _headline_score(self, text: str, explicit_headline: str) -> float:
        ...
        if not any(word in first.lower() for word in (
            "почему", "уров", "риск", "сценар", "вход", "движ", "данн", "контекст",
            "реш", "ошиб", "жду", "график", "индикатор", "цель", "структур",
        )):
            score -= 18.0
    ...
    def _structure(text: str, content_format: str) -> float:
        lowered = text.lower().replace("ё", "е")
        score = 35.0
        score += 20.0 if len([part for part in text.split("\n\n") if part.strip()]) >= 5 else 0.0
        score += 20.0 if all(marker in lowered for marker in ("вход:", "цели:", "стоп-лосс:", "r/r:")) else 0.0
        score += 15.0 if any(marker in lowered for marker in ("триггер", "сценарий a", "что я жду", "практический вывод", "ключевая граница")) else 0.0
