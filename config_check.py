...
def main() -> int:
    ...
    content_mode = os.getenv("CONTENT_MODE", "deterministic").strip().lower()
    ...
    enable_reach_gate = _bool("ENABLE_REACH_GATE", "0")
    ...
    min_quality = _number("MIN_POST_QUALITY", "80", float, 50, 100, errors)
    post_min = _number("POST_MIN_CHARS", "180", int, 120, 700, errors)
    post_max = _number("POST_MAX_CHARS", "760", int, 300, 1500, errors)
    min_interval = _number("MIN_GLOBAL_INTERVAL_MIN", "20", int, 20, 1440, errors)
    max_daily = _number("MAX_POSTS_PER_DAY", "72", int, 1, 72, errors)
    min_reach = _number("MIN_REACH_SCORE", "0", float, 0, 100, errors)
    ...
    if content_mode != "deterministic":
        warnings.append(
            "AI-режим повышает риск машинной однотипности; для охватов рекомендован CONTENT_MODE=deterministic"
        )
        if not mistral_key:
