...
def generate_chart(
    ...
    *,
    ...
    figure = None
    try:
        figure = plt.figure(figsize=(12, 7), facecolor="#0f1216")
        ...
        price_axis.plot(frame.index, frame["EMA50"], color="#4a90e2", linewidth=1.25, label="EMA50")
        price_axis.plot(frame.index, frame["EMA200"], color="#bd10e0", linewidth=1.0, linestyle="--", label="EMA200")
        price_axis.plot(frame.index, frame["BB_H"], color="#58616d", linewidth=0.7, linestyle=":")
        ...
        price_axis.text(
            0.012,
            0.965,
            f"LAST {_fmt_price(current_price)}",
        ...
        title = f"{basic.upper()}/USDT · 15m · {'LONG' if direction == 'long' else 'SHORT'}"
        price_axis.set_title(title, color="#e7ebf1", fontsize=13, fontweight="bold", pad=12)
        price_axis.set_ylabel("Price (USDT)", color="#c8ccd4", fontsize=9)
        volume_axis.set_ylabel("Volume", color="#c8ccd4", fontsize=8)
        price_axis.legend(loc="upper right", fontsize=7, framealpha=0.18, labelcolor="#c8ccd4")
