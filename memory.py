...
MEMORY_FILE = resolve_state_file("POST_MEMORY_FILE", "post_memory.json")
...
_BOILERPLATE_TOKENS = {
    ...
    "ретест", "подтверждение", "удержат", "закрепятся", "актуален",
}
...
class PostMemory:
    def __init__(self, path: Path = MEMORY_FILE, keep_days: int = 30, max_items: int = 160):
        self.path = Path(path)
        migrated = migrate_legacy_state(self.path, "post_memory.json")
        if migrated:
            logger.info("Migrated post memory from %s to %s", migrated, self.path)
    ...
    @classmethod
    def _editorial_text(cls, text: str) -> str:
        """Remove mandatory price-plan boilerplate before semantic comparison.

        Every valid post must repeat exact levels, labels and risk wording. Comparing
        those blocks as normal prose creates false positives, so similarity focuses
        on the headline and editorial explanation while structure is scored separately.
        """
        paragraphs = []
        for part in re.split(r"\n\s*\n", str(text)):
            ...
            if not clean or clean.startswith("#"):
                continue
            if "вход:" in lower and ("стоп-лосс:" in lower or "цели:" in lower):
                continue

            # Human Feed v6 expresses the trade contract as natural sentences
            # rather than a labelled block. Those sentences necessarily repeat
            ...
            trade_contract = has_price and (
                ...
                or any(
                    marker in lower
                    for marker in ("сценарий для меня отмен", "идею просто закрываю", "больше не актуален")
                )
            )
            if trade_contract:
                continue

            factual_context = has_price and any(
                marker in lower
                for marker in (
                    "за 15 минут", "за 45 минут", "15m-свеч", "15m свеч",
                    "объём последней", "объем последней", "объём свечи", "объем свечи",
                    "rsi ", "adx ", "btc-фон", "btc фон",
                )
            )
            if factual_context:
                continue
    ...
    def _features(cls, text: str) -> dict:
        editorial = cls._editorial_text(text)
        tokens = cls.semantic_tokens(editorial)
        return {
            "semantic": " ".join(tokens),
            "token_set": set(tokens),
            "trigrams": cls._ngrams(tokens),
            "openers": cls.paragraph_openers(editorial),
    ...
    def _compare_features(cls, left: dict, right: dict) -> float:
        ...
        score = (
            token_jaccard * 0.30
            + trigram_jaccard * 0.40
            + opener_jaccard * 0.20
            + structure_ratio * 0.10
        )
    ...
    def get_last_content_formats(self, n: int = 10) -> List[str]:
        return [str(item.get("content_format", "")) for item in self.items[-n:] if item.get("content_format")]
    ...
    def find_previous_symbol(self, symbol: str, n: int = 120) -> dict | None:
        ...
        for item in reversed(self.items[-n:]):
            if str(item.get("symbol", "")).upper() == target:
                return dict(item)
        return None

    def format_frequency(self, n: int = 30) -> dict:
        counts = {}
        for format_id in self.get_last_content_formats(n):
    ...
    def similarity_score(self, text: str, n: int = 36) -> float:
    ...
    def most_similar(self, text: str, n: int = 36) -> Tuple[float, str]:
        candidate = self._features(text)
        best_score = 0.0
        best_index = -1
    ...
    def is_similar(self, text: str, threshold: float = 0.52) -> bool:
