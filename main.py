...
COOLDOWN_MIN = int(os.getenv("COOLDOWN_MIN", "720"))
...
MIN_POST_QUALITY = float(os.getenv("MIN_POST_QUALITY", "80"))
...
def _choose_market_candidate(
    ...
    memory: PostMemory,
    primary_data: Dict[str, Dict[str, pd.DataFrame]],
) -> Optional[Tuple[MultiTimeframeIndicators, SignalScore, Optional[float], AttentionSnapshot]]:
    """Choose the setup with the strongest *current* attention, not only structure."""
    frequencies = memory.signal_type_frequency(24)
    last_signal_types = memory.get_last_signal_types(5)
    eligible: List[Tuple[float, MultiTimeframeIndicators, SignalScore, Optional[float], str, AttentionSnapshot]] = []
    ...
    for mtf, score in ranked:
        ...
        # Technical quality prevents garbage signals; current attention determines
        # whether the topic has a realistic chance to be opened in the feed now.
        adjusted_score = score.total * 0.46 + attention.score * 0.54 - diversity_penalty
        eligible.append((adjusted_score, mtf, score, funding, best_angle.id, attention))
        logger.info(
            "Candidate %s tech=%.1f attention=%.1f final=%.1f angle=%s "
            "15m=%+.2f%% vol=x%.2f extended=%s repeats=%s",
            mtf.symbol,
            score.total,
            attention.score,
            adjusted_score,
    ...
    _, mtf, score, funding, _, attention = eligible[0]
    return mtf, score, funding, attention
    ...
    *,
    ...
    for index, draft in enumerate(drafts):
        try:
            ...
            # Reward a strong first line, an underused editorial family and a
            # format that matches the live market state. Extended moves should
            # read as caution/analysis, not as a blind entry alert.
            editorial_bonus = 2.0 if draft.content_format not in recent_formats[-6:] else 0.0
            context_bonus = 0.0
            if attention.overextended:
                if draft.content_format in {"why_wait", "contrarian_take", "mistake_to_avoid", "signal_vs_trade"}:
                    context_bonus += 7.0
                if draft.content_format in {"setup_plan", "execution_protocol"}:
                    context_bonus -= 6.0
            if attention.score >= 65 and draft.visual_style == "pulse_card":
                context_bonus += 5.0
            if attention.volume_spike >= 2.0 and ("за 15 минут" in draft.headline.lower() or "объём" in draft.headline.lower()):
                context_bonus += 4.0
            adjusted_score = report.score - similarity_penalty - novelty_penalty + editorial_bonus + context_bonus
            logger.info(
                "Post candidate %s: format=%s visual=%s angle=%s quality=%.1f valid=%s "
...
def _run_once() -> int:
    ...
    chosen = _choose_market_candidate(ranked, btc, memory, primary_data)
    if chosen is None:
        logger.info("All candidates were rejected by BTC/funding safety filters")
        return 0

    best_mtf, best_score, funding, attention = chosen
    ...
    logger.info(
        "BEST %s tech=%.1f attention=%.1f direction=%s profile=%s trend=%.0f momentum=%.0f volume=%.0f "
