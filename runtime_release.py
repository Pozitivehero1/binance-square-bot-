...
def activate_release() -> None:
    ...
    # v11.9 is intentionally layered on top of the proven v11.8 distribution
    ...
    if not final_text_reasons("$ORCA: Оверсаттеринг момента. Cashtag @oracetrade"):
        raise RuntimeError("v11.9 malformed-language guard is incomplete")
    if os.environ.get("BOT_VERSION") != "v11.9":
        raise RuntimeError("v11.9 version defaults were not activated")
    ...
    print("[v11.9] cumulative release verified: distribution recovery + writer hardening active")
